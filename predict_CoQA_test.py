...
def main():
    ...
    CoQAEval = CoQAEvaluator("CoQA/coqa-test-v1.0.json")
    ...
    with open("CoQA/coqa-test-v1.0.json", "r", encoding="utf8") as f:
...
def load_dev_data(opt): # can be extended to true test set
    with open(os.path.join(args.dev_dir, 'test_meta.msgpack'), 'rb') as f:
        meta = msgpack.load(f, encoding='utf8')
    embedding = torch.Tensor(meta['embedding'])
    assert opt['embedding_dim'] == embedding.size(1)

    with open(os.path.join(args.dev_dir, 'test_data.msgpack'), 'rb') as f:
