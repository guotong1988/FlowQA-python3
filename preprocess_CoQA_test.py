...
trn_file = 'CoQA/coqa-train-v1.0.json'
dev_file = 'CoQA/coqa-test-v1.0.json'
...
if True:
    ...
    with open('CoQA/train_meta.msgpack', 'rb') as f:
    ...
    with open('CoQA/train_data.msgpack', 'rb') as f:
        result = msgpack.load(f, encoding='utf8')

    trQ_ids=result['question_ids']
    trC_ids=result['context_ids']
    trC_features = result['context_features']
    trC_tag_ids = result['context_tags']
    trC_ent_ids = result['context_ents']
    train_context = result['context']
    train_context_span = result['context_span']
    first_question = result['1st_question']
    ...
    trC_tokens =  result['context_tokenized']
    trQ_tokens = result['question_tokenized']
