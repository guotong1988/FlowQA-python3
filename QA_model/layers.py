...
class DeepAttention(nn.Module):
    ...
    def forward(self, x1_word, x1_abstr, x2_word, x2_abstr, x1_mask, x2_mask, return_bef_rnn=False, previous_hiddens=None):
        ...
        x1_att = torch.cat(x1_word + x1_abstr, 2) #[10,384,1400]
        x2_att = torch.cat(x2_word + x2_abstr[:-1], 2) #[10,20,1400]
        x1 = torch.cat(x1_abstr, 2) #[10,384,500]

        x2_list = x2_abstr #[10,20,250]*3
        for i in range(len(x2_list)):
            attn_hiddens = self.int_attn_list[i](x1_att, x2_att, x2_mask, x3=x2_list[i], # [10,384,250]
...
class GetSpanStartEnd(nn.Module):
    ...
    def forward(self, x, h0, x_mask):
        ...
        x_mask = batch * len
        """
        st_scores = self.attn(x, h0, x_mask) # [10,384]
        # st_scores = batch * len

        if self.rnn is not None:
            ptr_net_in = torch.bmm(F.softmax(st_scores, dim=1).unsqueeze(1), x).squeeze(1) # [10,250]
        ...
        return st_scores, end_scores # [10,384]
