...
class QAModel(object):
    ...
    def update(self, batch):
        ...
        if self.opt['cuda']:
            ...
            answer_start = batch[10].cuda(non_blocking=True)
            answer_end = batch[11].cuda(non_blocking=True)
            answer_class = batch[12].cuda(non_blocking=True)
            rationale_start = batch[13].cuda(non_blocking=True)
            rationale_end = batch[14].cuda(non_blocking=True)
        else:
            inputs = [e for e in batch[:9]]
            overall_mask = batch[9]

            answer_start = batch[10]
            answer_end = batch[11]
            answer_class = batch[12]
            rationale_start = batch[13]
            rationale_end = batch[14]

        # Run forward
        # output: [batch_size, question_num, context_len], [batch_size, question_num]
        score_start, score_end, score_class = self.network(*inputs)
        ...
        all_no_span = (answer_class != 3)
        answer_start.masked_fill_(all_no_span, -100) # ignore_index is -100 in F.cross_entropy
        answer_end.masked_fill_(all_no_span, -100)
        rationale_start.masked_fill_(all_no_span, -100) # ignore_index is -100 in F.cross_entropy
        rationale_end.masked_fill_(all_no_span, -100)

        for i in range(overall_mask.size(0)):
            q_num = sum(overall_mask[i]) # the true question number for this sampled context

            target_s = answer_start[i, :q_num] # [q_num]
            target_e = answer_end[i, :q_num] # [q_num]
            target_c = answer_class[i, :q_num] # [q_num]
            target_s_r = rationale_start[i, :q_num] # [q_num]
            target_e_r = rationale_end[i, :q_num] # [q_num]
            target_no_span = all_no_span[i, :q_num] # [q_num]

            # single_loss is averaged across q_num
            # [10,4]vs[10]
            # [10,384]vs[10]
            # [10,384]vs[10]
            single_loss = (F.cross_entropy(score_class[i, :q_num], target_c) * q_num.item() / 15.0
                         + F.cross_entropy(score_start[i, :q_num], target_s) * (q_num - sum(target_no_span)).item() / 12.0
                         + F.cross_entropy(score_end[i, :q_num], target_e) * (q_num - sum(target_no_span)).item() / 12.0)
